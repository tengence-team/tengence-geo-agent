...
test('auto-init: first connect creates 18 tables + user_version=4 (idempotent)', async () => {
  // getDb() already performs lazy initialization internally; a second ensureSchema
  // is idempotent and returns applied:false
  const db = sqlite.getDb();
  assert.equal(sqlite.dbStatus().tableCount, 18);
  assert.equal(sqlite.dbStatus().schemaVersion, 4);
